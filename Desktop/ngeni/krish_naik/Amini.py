...
load_dotenv()
api_key = os.getenv("GROQ-_API_KEY")
tavil = os.getenv("TAVILY")
#Define diagnostic and action tools
tools =[TavilySearchResults(max_results=3, tavily_api_key=tavil)]  #search tool used for subtask execution
...
planner = planner_prompt|ChatGroq(model="meta-llama/llama-4-scout-17b-16e-instruct", api_key=api_key).with_structured_output(Plan)
#Re-planning step
replanner_prompt = ChatPromptTemplate.from_template(
    """For the given objective, come up with a simple step by step numbered plan.\
        This plan should involve individual tasks, that if executed correctly will yeild the correct answer. Do not add any superfluous steps.\
            The result of the final step should be the final answer. Make sure that each step has all the information needed-do not skip steps.
...
replanner = replanner_prompt|ChatGroq(model="meta-llama/llama-4-scout-17b-16e-instruct", api_key=api_key).with_structured_output(Act)
...
async def run_plan_and_execute():
    #Input from the user 
    inputs = {"input":"grace weighs 125 pounds. Alex weighs 2 pounds less then 4 times what Grace weighs. What are their combined weights in pounds?"}
